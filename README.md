# data_storage MCP Server

The MCP server providing data storage tools for SuperBuilder
...
# The executable will be created in dist/data_storage-mcp-server.exe
```

### 4. Production Usage
```powershell
# Run the built executable (using stdio protocol)
.\dist\data_storage-mcp-server.exe start

# Check status
.\dist\data_storage-mcp-server.exe status --json

# Stop server
.\dist\data_storage-mcp-server.exe stop
```
...
async def my_custom_tool(input_param: str) -> str:
    ...
    return f"Processed: data_storage"
```
...
# Run specific test
python -m unittest test_server.TestData_storageServer.test_server_version
```
...
Copy-Item .\dist\data_storage-mcp-server.exe C:\MyServers\

# Run on target system
C:\MyServers\data_storage-mcp-server.exe start
```
...
.\dist\data_storage-mcp-server.exe start                              # Start with configured protocol
.\dist\data_storage-mcp-server.exe start --protocol sse --port 8080    # Override to use SSE protocol with custom port
.\dist\data_storage-mcp-server.exe status --json
.\dist\data_storage-mcp-server.exe stop
```
...
├── data_storage_server.spec # PyInstaller specification
