...
# Message templates
SERVER_MESSAGES = {
    'server_start': 'data_storage MCP server starting' + (' on port {port}' if 'stdio' != 'stdio' else ''),
    'server_started': 'data_storage MCP server started successfully' + (' on port {port}' if 'stdio' != 'stdio' else ''),
    'server_stopped': 'data_storage MCP server stopped',
    'server_start_fail': 'Failed to start data_storage MCP server',
    ...
    'status_running': 'data_storage MCP server is running',
    'status_stopped': 'data_storage MCP server is not running',
...
def create_response(success, message, data=None, exit_code=0, current_port=None, current_protocol=None):
    ...
    response = {
        ...
        "protocol": current_protocol or data_storage_PROTOCOL,
    }
...
# Generated by McpServerGen.py on 2025-11-17 16:01:17
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
data_storage_PORT = int(os.getenv('MCP_DATA_STORAGE_PORT', 7905)) if 'stdio' != 'stdio' else None
data_storage_PROTOCOL = os.getenv('MCP_DATA_STORAGE_PROTOCOL', 'stdio')
...
def start_server(port=None, protocol=None):
    ...
    if protocol is None:
        protocol = data_storage_PROTOCOL
    
    # For stdio protocol, we don't need a port
    if protocol == 'stdio':
        port = None
    
    if (protocol == 'stdio' and mcp is None) or (protocol != 'stdio' and port != data_storage_PORT):
        # Create fresh FastMCP instance with current port and protocol
        if protocol == 'stdio':
            mcp = FastMCP("data_storage")
        else:
            mcp = FastMCP("data_storage", port=port)
...
def stop_server(json_mode=False, port=None):
    ...
    if data_storage_PROTOCOL == 'stdio':
        port = None
                    ...
                    try:
                        ...
                        if proc.info['name'] and 'python' in proc.info['name'].lower():
                            ...
                            if (cmdline and 
                                ...
                                any('data_storage' in str(arg).lower() for arg in cmdline)):
...
def is_server_running():
    ...
    return _server_running and is_port_in_use(data_storage_PORT)
...
if __name__ == "__main__":
    ...
    parser = ArgumentParserWithExceptions(
        description="The MCP server providing data storage tools for SuperBuilder",
...
Examples:
  ...
  python server.py version
    ...
    try:
        ...
        current_port = data_storage_PORT
        current_protocol = data_storage_PROTOCOL
        if args.port:
            current_port = args.port
            os.environ[f'MCP_DATA_STORAGE_PORT'] = str(current_port)
        if args.protocol:
            current_protocol = args.protocol
            os.environ[f'MCP_DATA_STORAGE_PROTOCOL'] = current_protocol
        ...
        if current_protocol == 'stdio':
            ...
            mcp = FastMCP("data_storage", port=current_port)
        ...
        current_port_safe = locals().get('current_port', data_storage_PORT)
        current_protocol_safe = locals().get('current_protocol', data_storage_PROTOCOL)
        response, exit_code = create_response(
            True, 
            format_message('interrupt'),
